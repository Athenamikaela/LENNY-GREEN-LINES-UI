	...
	function Gui.Create_Tab(options)
		...
		function Tab.Create_Section()
			...
			function Section.Create_DropToggle(options)
				...
				-- layout counters for insert order (negative = prepend/top, positive = append/bottom)
				Module._layoutIndex = 0
				Module._prependIndex = 0
				...
				Module["5c"] = Instance.new("ScrollingFrame", Module["5a"]);
				Module["5c"]["Active"] = true;
				...
				Module["5c"]["ScrollBarImageTransparency"] = 0.8;
				Module["5c"]["ScrollBarThickness"] = 6;
				Module["5c"]["AutomaticCanvasSize"] = Enum.AutomaticSize.Y;
				...
				-- Primary dropdown moved out of the module settings so additional
				-- dropdowns added to the module don't get obscured.
				-- Only create the primary dropdown if caller provided options.
				if options and options.options then
					Module["5e"] = Instance.new("TextButton", Target_Section);
				...
				-- keep the primary dropdown header compact (same as other dropdowns)
				Module["5e"]["Size"] = UDim2.new(0, 216, 0, 27);
				Module["5e"]["BackgroundTransparency"] = 1;
				Module["5e"]["Name"] = [[Dropdown]];
				Module["5e"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
				Module["5e"]["Text"] = [[]];
				-- hide the built-in primary dropdown (moved out earlier); it should not be visible
				Module["5e"]["Visible"] = false

				Module["5f"] = Instance.new("UIPadding", Module["5e"]);
				-- ensure primary dropdown occupies the baseline layout order
				Module["5e"]["LayoutOrder"] = 0
				...
				if Library.Flags[Module.flag] then
					local Background_Tween = Ts:Create(Module["88"], TweenInfo.new(0.4), { ImageTransparency = 0.5 })
						...
						pcall(function() Module["5c"].CanvasPosition = Vector2.new(0,0) end)
					else
				...
				UserInputService.InputBegan:Connect(function(input, gpe)
					if Module.WaitingForKey and not gpe then
						if input.UserInputType == Enum.UserInputType.Keyboard then
							local key = input.KeyCode.Name
							if #key == 1 and key:match("%a") then
								Module["8d"].Text = key:upper()
								Module["8d"].Visible = true
								Module["90"].Visible = false
					...
					elseif Module.Keybind and input.KeyCode == Module.Keybind and not gpe then
						Module:Toggle()
					end
				end)
					...
					local function toggleDD(state)
						...
						if open then
							...
							for _, child in pairs(parent:GetChildren()) do
								if child ~= DD_Container and child:IsA('GuiObject') then
									local ok, ord = pcall(function() return child.LayoutOrder end)
									if ok and type(ord) == 'number' then
							...
							if nextChild then
								...
								RunService.Heartbeat:Wait()
								local ok1, containerPos = pcall(function() return DD_Container.AbsolutePosition end)
								local ok2, nextPos = pcall(function() return nextChild.AbsolutePosition end)
								local containerAbsY = (ok1 and containerPos and containerPos.Y) and containerPos.Y or 0
								local nextAbsY = (ok2 and nextPos and nextPos.Y) and nextPos.Y or 0
								...
								-- debug: print which sibling we adjusted for (helps debugging layout issues)
								pcall(function()
									warn('toggleDD: nextChild=', nextChild.ClassName, 'LayoutOrder=', nextChild.LayoutOrder, 'overlap=', math.floor(overlap))
								end)
							end
