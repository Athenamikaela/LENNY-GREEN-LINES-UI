			...
			function Section.Create_DropToggle(options)
				...
				function Module.Create_Dropdown(self, options)
					...
					local function toggleDD(state)
						...
						open = state
						if open then
							-- expand the whole container (header + options) so siblings slide down
							Ts:Create(DD_Container, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(0, 216, 0, 112) }):Play()
							Ts:Create(DD.Arrow, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Rotation = -90 }):Play()
						else
							-- collapse container back to header-only height
							Ts:Create(DD_Container, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(0, 216, 0, 27) }):Play()
							Ts:Create(DD.Arrow, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Rotation = 0 }):Play()
						end
					end

					DD.Frame.MouseButton1Click:Connect(function()
						toggleDD()
					end)
